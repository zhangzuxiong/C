...
//查找--按照值查询返回查找的地址
Order* searchValueReturnAddressOrderList(const OrderList list, const Order order) {
	if (judgeEmpotyOrderList(list))
	{
		printf("双链表已经为空，不能查找\n");
		return NULL;
...
//打印
void printOrderList(const OrderList list,const User user) {
	if (judgeEmpotyOrderList(list))
	{
	...
	while (temp != NULL)
	{
		...
		printf("订单号:%d,用户姓名:%s,联系电话:%s,收件地址:%s,下单时间:%s\n", 
			temp->order.order_num,user.name,user.phone,user.address,temp->order.order_time);
...
//将订单信息保存到文件
void saveAllOrderInfo(const OrderList list,const User user) {
	...
	{
		//写订单号、用户id、下单时间
		fprintf(file, "%d\t%d\t%s\t",order->order.order_num,user.ID,order->order.order_time);
