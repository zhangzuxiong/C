...
void insert_position_Single_Linked_List(SL_List* p, const int position, const int num) {
	...
	*/
	if (position == 1)
	{
		...
		node->next_pointer = p->head.next_pointer;

		// 赋值
		node->data = num;

		// 将头指针 指向 新结点，这是单链表的 新的 第一个有效结点
		p->head.next_pointer = node;
	}
	else
	{
		// 需要一个临时指针变量来定位
		Node* index = p->head.next_pointer;
...
int lookup_position_Single_Linked_List(const SL_List list, const int position) {
	...
	Node* index = list.head.next_pointer;
...
int main()
{
	...
	insert_position_Single_Linked_List(&list, 1, 1);
	insert_position_Single_Linked_List(&list, 1, 2);
	insert_position_Single_Linked_List(&list, 1, 3);
	insert_position_Single_Linked_List(&list, 1, 4);
	printf("\n--------------------\n\n");
	insert_position_Single_Linked_List(&list, 5, 5);
	printf("\n--------------------\n\n");
	print_Single_Linked_List(list);
	printf("\n--------------------\n\n");

	printf("删除节点元素为:%d", delete_position_Single_Linked_List(&list, 1));
	printf("删除节点元素为:%d", delete_position_Single_Linked_List(&list, 4));
	printf("删除节点元素为:%d", delete_position_Single_Linked_List(&list, 4));
	printf("删除节点元素为:%d", delete_position_Single_Linked_List(&list, 0));
	print_Single_Linked_List(list);
	printf("\n--------------------\n\n");
